...
## Cycle 6 / MEMBER-ENRICH — class + discipline (+belt +status) on member cards, Member-360, roster (2026-06-19)

**Mandate:** zero schema, read-time only. Surface, *at a glance*, the discipline + active class(es) a member trains (+ belt + membership status) on the surfaces that had the data one join away but didn't show it. Keep the info area **extension-ready** (more per-member fields incoming).

### What surfaced where
- **Member-list cards** (`students/components/student-list.tsx`) — a new `member-info` area: ordered chips **discipline · class (Dumbbell) · membership-status**, beside the existing belt badge. Status tone: active=green, expiring=amber, lapsed=red.
- **Member-360 enrollments panel** (`students/[id]/page.tsx`) — each registration row now shows **class + discipline + schedule day/time** (`reg-discipline-schedule`), where before it showed only the class name + fee.
- **Class roster** (`classes/[id]/ClassDetail.tsx`) — each enrolled student's row now carries **belt + discipline chip(s)** (`roster-info` / `roster-discipline`), so the floor view is informative.

### The discipline join (the thing that was missing)
One shared helper — `src/lib/members/enrichment.ts` `getMemberEnrichment(supabase, gymId, studentIds, locale)` — does the `class → disciplines` join the surfaces lacked, **over BOTH membership paths**: `class_enrollments` (the attendance roster, which has no `gym_id` → scoped by the already-gym-scoped student ids + RLS) **and** active `class_registrations` (the B2 flow, `gym_id`-scoped), unioned + de-duped, plus `student_memberships` for status. One batched read (`Promise.all`) per surface, keyed by student id. **Unioning both paths was load-bearing:** the run seed enrolls members via `class_enrollments` with no registration, so a registration-only helper would have rendered empty for every seeded member. The Member-360 panel additionally got `disciplines:discipline_id (…)` + `class_schedules (…)` added to its own reg query (it owns the schedule, which the shared helper doesn't carry).

### The extension point (more fields incoming)
The info area is **a small ordered set of labeled chips fed by one read** — not hard-coded one-offs. The drop-in point is flagged in three places (the `EXTENSION POINT` comments in `enrichment.ts`, the card, and the roster): to add the next per-member field, extend `MemberInfo`, select the extra column in this one query, and render one more chip — **no new per-field plumbing, no extra round-trip, no new query per surface.**

### CI evidence (behavior, not tsc)
- **No migration** (zero schema). `tsc` + `next build` clean; i18n ar/en/fr **2161 keys ×3, 0 missing/0 orphan** (`students.membership.{active,expiring,lapsed}`), RTL.
- **E2E gate `27828734073` — SUCCESS, 92 passed (34.4m), 0 failed** — https://github.com/TechStack2/proline-gym-platform/actions/runs/27828734073
  - `member-enrich · member-list cards`: Karim's card renders `member-info` with discipline + class + belt chips and a membership-status chip is present.
  - `member-enrich · Member-360 + roster`: a fresh class (Mon/Wed/Fri @ 19:00) + register Karim (request→approve = active, which projects the roster) → his Member-360 reg row shows the class's discipline (cross-checked against the class-detail header) **and** `19:00`; the class roster shows Karim with belt + discipline chip.
  - `member-enrich · /ar`: the enriched chips render in Arabic with no unresolved `students.membership` key / `MISSING_MESSAGE`.
  - `member-enrich` project appended at the END (runs LAST); rest of the suite unaffected.

### **class + discipline + belt + status visible on member cards / roster / Member-360: PASS**

### DRAG READ
**The fixture taught the real lesson, and it's about read-time enrichment in a serial shared-gym suite.** First CI run failed one assertion (88/1): I'd asserted Karim's *first* class chip says "Muay Thai" and that his belt is "White" — both true of the *seed*, neither true by the time `member-enrich` runs **last**. By then `ia-nav`/`fd1`/`fd2`/`drill360` have each registered Karim to their own classes (so his first chip was `IA2 Class …`), and `activity-loop` has **promoted his belt** off White via `promote_student`. The product was correct the whole time — the chips rendered, the discipline showed, the locator was stable; my test was asserting *seed values* against a *heavily-mutated* member. The fix is the durable rule: **a last-running spec must assert that the enrichment RENDERS (chip/belt presence), not specific values or order**, because every prior spec is free to add classes and advance belts on the shared fixture. Where a value DID need checking (the Member-360 reg discipline), I anchored it to a *deterministic* source created in the same test — the fresh class's own `class-discipline` header — rather than to the seed.

**Second, the union was the design call, not an afterthought.** "What class is this member in?" has two truths in this schema — the attendance roster (`class_enrollments`) and the billing relationship (`class_registrations`) — and they don't imply each other (the seed populates only the former; the B2 flow only the latter, until `approve` projects one into the other). A card that read only registrations would be blank for a seeded, attending member; one that read only enrollments would miss request→approve members until their roster row lands. Surfacing "informative at a glance" honestly meant reading both. Net: read-time only, zero schema, one batched read per surface, and one extension point so the next round of operator feedback is a chip, not a re-plumb.

---

## Cycle 6 / STABILIZE-E2E — flaky-gate stabilization (2026-06-19)

**Mandate:** the behavior gate was *unreliable* — pt1/pt2/g2/adm1 failed intermittently regardless of the change under test, costing re-run cycles and letting a real regression hide behind "probably just flake." Pin the flakes at the ROOT so green means green. **Test/infra only — zero product-code/schema change, zero assertion weakening, no global-retry bump** (`playwright.config.ts` untouched). The one shared lever added: `helpers.ts` `untilConsistent(action)` — re-runs a **reads-only** action via `expect(action).toPass()` until an eventually-consistent read reflects the write.

### Per-test root race + fix
| Test | Root race | Fix (waits/fixtures only) |
|---|---|---|
| **pt1:102** (use→refill; failed at 111 / 164) | **Read-after-write**: the desk sale (committed in test 1) and the Extend RPC write lag the coach-roster / member-file read (PostgREST replica + realtime); a one-shot `goto`+15s sat on a stale snapshot. | `untilConsistent` re-navigates the coach roster and the member file until the just-written row surfaces. `pt-log` clicks stay **single** (idempotency preserved — never wrapped in the retry). |
| **pt2:77/153/218** (`pt-slot` not found ≤20s) | **availability-publish → slot-compute → render**: the slot list intermittently rendered empty within the one-shot wait. | `openSlots()` re-opens the Book modal (re-reading availability) until a `pt-slot` renders. The race-loser keeps a genuinely **stale** list — both lists load *before* A books, so the slot-taken path is preserved. |
| **g2:66** (banner won't drain) | The product's reconnect flush fires on the `'online'` event **exactly once with no retry** → a single transient `saveAttendance` failure left the queue stuck (the "fails then recovers on retry" signature). | `reconnectAndDrain()` re-fires the `'online'` event (quick offline→online toggle) to re-run the **idempotent** flush (keyed upsert) until the banner drains. No reload (roster stays open); persistence + idempotency assertions unchanged. |
| **adm1** (disciplines; + the class-lifecycle landing reads) | **ISR/landing cache + SSOT read-after-write**: a publish/edit/archive/discipline write is eventually-consistent on the anon landing and the class-wizard chips; a one-shot `openLanding` + in-place poll never re-fetched. | `untilConsistent` re-opens the landing / class wizard until it reflects the write (the chip-gone case is guarded by "chips loaded first" so it can't false-pass on an empty wizard). |

No `expect` was deleted or loosened — every removed inline assertion was **moved into** these helpers and runs on every attempt; the per-attempt inner timeouts shrank but sit inside a *longer* `toPass` budget (net wait is larger, not smaller).

### Consecutive-green evidence (the unusual acceptance bar — prove STABILITY)
After fixing g2 (see drag-read), the full e2e suite was dispatched back-to-back and went green **3× consecutively**, with all four named projects green in every run:
- **`27835896275`** — SUCCESS, 92 passed / 0 failed (36.0m) — https://github.com/TechStack2/proline-gym-platform/actions/runs/27835896275
- **`27837680305`** — SUCCESS, 92 passed / 0 failed (33.6m) — https://github.com/TechStack2/proline-gym-platform/actions/runs/27837680305
- **`27839259084`** — SUCCESS, 0 failed (36.6m) — https://github.com/TechStack2/proline-gym-platform/actions/runs/27839259084

(pt1/pt2/adm1 were additionally green in the earlier run `27833571649`, i.e. 4 consecutive for those three; g2 — the one that needed a second pass — is green across all three back-to-back runs above, the previously-timing-out `g2:offline mark` now finishing in ~18–20s vs the 220s hang.)

### **pt1/pt2/g2/adm1 stable across 3 consecutive runs; no assertion weakened, no global-retry bump, zero product/schema change: PASS**
...
## Cycle 6 / PORTAL-FND — portal design-system + shell foundation (2026-06-19)

**Mandate:** the owner loved the staff-side premium-360, then flagged that the **self-service portals don't feel like the premium app** — the member portal is "themeless," cards aren't drillable; the coach portal is "thin / unorganized / overlapping." Audited root cause: the staff dashboard adopted the brand design system + the `ActionCard`/`DrillDetails` kit; the **portals (`coach/*`, `portal/*`) never did** (1 file across both used `ui/*`, 0 used the drill kit). This slice closes that at the **foundation level only** — theme + shell + kit-availability — which fixes the cross-cutting "feel" complaints and **unblocks** the two portal-360 feature builds. **Presentation + shell only; zero schema; zero new features/data.**

### What was themed / unified
- **New portal kit** — `src/components/portal/portal-kit.tsx`, every primitive styled against `docs/design-system.md` (not vibes):
  - **`PortalContent`** — the single shell content wrapper now used by **both** portal layouts (`CoachLayoutClient` + `PortalLayoutClient`). Two jobs: (1) a centred **max-width reading column** (`mx-auto w-full max-w-3xl`) so self-service pages stop sprawling full-bleed on desktop — a member/coach gets a focused column, not the 7-workspace admin grid; (2) the **FD-2 PWA-footer fix** the staff shell got but the portals never did.
  - **`PortalCard` / `PortalCardTitle`** — the documented `rounded-2xl bg-white border border-gray-100 shadow-sm` card anatomy, **composed over the `ui/*` `<Card>`** (so portals now literally render via the design-system kit) + the `text-sm font-semibold` section title with a **brand-red** (`text-primary-600`) icon.
  - **`PortalEmpty`** — the documented empty-state (icon `text-gray-300` → one-liner `text-gray-400` → optional single action).
- **Member home (`portal/page.tsx`)** + **Coach home (`coach/page.tsx`)** adopt the kit — self-view / waiver / membership / belt / trials / class cards / stat tiles / empty states all moved onto `PortalCard`/`PortalCardTitle`/`PortalEmpty`. **Every `data-testid` and every `t(...)` string preserved** (self-view + self-membership/pt-remaining/next-class, portal-waiver(+chip), kid-switcher, portal-lifecycle-banner, coach-home-trials/-trial-row, "coach profile not found" tri-lingual). No new copy → no i18n surface added.
- **AX-1 per-shell identity left intact** — the PWA `themeColor` (staff `#cd1419` / coach `#111111` / portal `#0e7490`) and the labelled shell badges are **test-locked (`ax1-ar.spec.ts`) and documented** as the per-ROLE identity layer; the brand-red tokens apply to **content accents + primary CTAs**, per `design-system.md` ("the accent never replaces brand red on primary CTAs").

### Kit-availability proof-of-use
The member home **recent-attendance** section (previously a bespoke white box, untested) now renders via the staff app's **drillable-card framework**: an **`ActionCard`** headline (count badge in brand red; collapses to a single ✓ line on a quiet day) wrapping a **`DrillDetails`** whose rows are the member's own recent sessions, each **drilling into `/portal/schedule`**. Uses the data already fetched — **no new query, no new feature**. This confirms `ActionCard` + `DrillDetails` import + render in portal context with **no staff-only assumptions**, so the portal-360 builds get drillable cards without re-plumbing.

### Before / after of the shell
- **Before:** `coach/*` + `portal/*` pages each rendered their own ad-hoc `rounded-2xl bg-white p-4 shadow-sm` boxes with **no max-width** (full-bleed sprawl on desktop = "thin / unorganized"), **no border** (flat/"themeless"), and **no bottom clearance** — the last rows of every page hid behind the `fixed bottom-0` mobile `NativeTabBar` (= "overlapping"). The drill kit was absent portal-side.
- **After:** both shells wrap children in `PortalContent` → one consistent, centred, **non-overlapping** column with the documented spacing across **every** portal/coach page at once; the home pages carry the brand card anatomy + brand-red section accents; the drillable kit renders portal-side.

### Verify (local, pre-CI)
- `tsc --noEmit` clean · `next build` clean (all `coach/*` + `portal/*` routes compiled; SSG prerender of `/[locale]/coach` + `/[locale]/portal` succeeded) · `vitest` 7/7 · dev server compiles + serves `/{en,ar,fr}/{portal,coach}` with **307** auth-redirects (no 500, kit module resolves) · scoped `git add` (build-regenerated PWA artifacts reverted).
- **E2E CI green on `prompt-portal-fnd`:** run **`27849957822`** — **92 passed / 0 failed (28.9m)** — https://github.com/TechStack2/proline-gym-platform/actions/runs/27849957822. The full suite (incl. `student`/member-portal, `coach`, `f3` member+guardian waiver sign-from-portal, `ax1` /ar shell-identity, `member360` self-view, `b3` kid-switcher) is green against my change.
- **First run `27847545200`** (90 passed / 1 flaky / 1 failed) hit two **pre-existing flakes in surfaces this slice did not touch** — the re-run cleared both: (1) **f3** `signature-pad` `data-has-ink=false` on the *re-sign* (the *first* sign in the same test passed → the `portal-waiver` card change is fine; the pad lives in `src/components/shared/`, untouched, and draws via `boundingBox()`-relative coords in a modal overlay outside the `PortalContent` column), the 1-retry budget then doomed by f3's own non-idempotency (it bumps the template to v2 at L93 before failing, so the retry's `expect v1` can't pass); (2) **ax1** landing-page CLS 0.225 > 0.1 (flaked → passed on retry; landing isn't in this diff). My commit touched zero waiver/signature/settings/landing files; f3 was green at 48s in the prior stabilize run `27839259084`. No test was weakened.

### **coach + member portals adopt brand theme + consistent shell + drill kit available; no feature/regression: PASS (E2E green — run 27849957822, 92/0)**

### DRAG READ
**The "themeless" complaint was half mislabelled, and naming the real cause is the deliverable.** Reconciled against `docs/design-system.md`, the portal pages mostly *already* followed the documented card anatomy — what they lacked was (a) the `border border-gray-100` that lifts a card off a white page, (b) a **max-width column** (the desktop sprawl read as "thin/unorganized"), and (c) the **FD-2 footer clearance** (the literal "overlapping"). So the highest-leverage fix was the **shell wrapper**, not a per-page repaint: one `PortalContent` themed **all** portal/coach pages' spacing/width/overlap in two edits, and the per-page kit adoption was then reserved for the two homes the owner actually saw. The "coach is overlapping" line was a **real, reproducible product bug** (staff got the `pb-[calc(4rem+safe-area)]` fix in FD-2; the portals were never patched) — fixed at the shell, so it can't regress per-page.

**The deliberate non-change is the per-shell accent.** The instinct (and a literal reading of "adopt the crimson brand tokens") was to recolour the portal/coach active-nav + accent bar to crimson. But `ax1-ar.spec.ts` **asserts** the per-shell PWA theme-colors + badges, and `design-system.md` documents teal/gold as the intentional per-ROLE identity layered over brand red. So crimson went into **content accents + CTAs** (where the doc says it belongs) and the shell identity stayed — "premium brand feel" via the kit + polish, not by erasing a tested, documented layer. The `ui/*` `<Card>` is `rounded-xl` while the doc mandates `rounded-2xl`, and `<Card>` is used by 18 staff files — so rather than mutate the shared primitive (staff blast radius), `PortalCard` **composes** `<Card>` and overrides `rounded-2xl border-gray-100` via `twMerge`: portals render through the kit (satisfies "renders via a `ui/*` component") **and** match the documented anatomy, with zero staff risk.

**Where the portal-360 feature builds attach (the whole point of this foundation):** (1) the **Member-360-portal** build docks new drillable cards into `portal/page.tsx` using the now-proven `ActionCard`/`DrillDetails` pattern (the recent-attendance card is the worked example) — and should give the **guardian `KidDashboard`** (the per-kid home, intentionally left on its own markup this slice) the same kit pass; (2) the **Coach-360-portal** build docks the coach's own 360 hub onto `coach/page.tsx`'s kit + the `PortalContent` column. Both inherit the consistent shell for free; neither needs to re-theme or re-plumb. Remaining portal subpages (`billing/pt/progress/schedule/profile/classes`, `coach/{students,pt,attendance,profile,trials}`) already get the shell's width/overlap/spacing fix; their per-page kit adoption is feature-build scope, not foundation.

### Addendum — the foundation gets its OWN e2e (post-review) + the modal-positioning root cause it exposed

The first green run only proved *no regression*; nothing verified the foundation itself (the review's catch). Added **`e2e/portal-fnd.spec.ts`** (Playwright project `portal-fnd`, appended after `off1`, runs LAST — merged `origin/main` into the branch first so the array genuinely ends at `off1` and there's no fused-block conflict on merge). Three tests, the testable structure (the "premium feel" stays the operator's eye):
1. **member (`student@`) + coach (`coach@`) homes** render the ONE unified shell (`portal-shell` from portal-kit's `PortalContent`) on **desktop AND mobile** — exactly once (no double-shell), settled **within the viewport** (poll the live rect to wait out the PageTransition entrance slide), the desktop shell a **capped reading column** (≤ max-w-3xl, not full-bleed), with a ui/* card token (`portal-card`) + a brand-red token (`portal-brand`, asserted via computed color).
2. a `src/components/ui/*` `<Card>` renders portal-side + the **ActionCard + DrillDetails** kit renders (`card-portal-recent-attendance` + `portal-attendance-drill`) — the proof-of-use, deterministic because `activity-loop` marks Karim present/absent earlier in the run.
3. `/ar` portal is **RTL-clean**: `dir=rtl`, no `MISSING_MESSAGE`.

Test-only `data-testid`s added to portal-kit (`portal-shell`, default `portal-card` that never clobbers a caller's testid, `portal-brand`) + an optional testid on the coach brand StatCard. No behavior change (only visual tweak: the kit title icon now uses the exact brand `#cd1419`).

**Then `portal-fnd` went green but f3 (waiver) started failing ~4/5 on the branch while it was 4/4 green on main.** Root-caused + fixed (it's THIS slice's CI flakiness, not a pre-existing one): **a `transform` ancestor makes a `position:fixed` descendant resolve against that ancestor's box, not the viewport.** Portal & coach pages wrap content in `PageTransition`, whose settled inner div keeps `transform: translateX(0)`; the waiver modal (`fixed inset-0`) therefore centred over the **full page height**, so on a scrolled/tall portal page the signature canvas landed off-screen and the Playwright draw missed (`data-has-ink=false`). The exact page height — which PORTAL-FND's card swaps nudged — tipped it, hence the green-on-main / flaky-on-branch split. **Fix:** `createPortal` the WaiverSign modal JSX to `document.body` (mount-guarded) → viewport-fixed everywhere. No-op for staff (the desktop dashboard has no `PageTransition`, so it was already viewport-fixed); same testids/markup; onboarding embeds `WaiverConsentFields` directly (not this modal), so it's untouched. After the fix f3 is green again (40.4s).

### **E2E green WITH the foundation test: run `27866601210` — 98 passed / 0 failed (33.5m; the 1 "flaky" was g1's WhatsApp link timeout, self-healed on retry — unrelated).** https://github.com/TechStack2/proline-gym-platform/actions/runs/27866601210 · `portal-fnd` 3/3 ✓, f3 member+guardian ✓.
...
## Cycle 6 / STABILIZE-2 — f3 + ax1 pinned (2026-06-20)

**Mandate:** STABILIZE-E2E pinned pt1/pt2/g2/adm1; two residual flakes remained and taxed every run — `f3` signature-pad `data-has-ink` (canvas draw) and `ax1` landing CLS (0.225 > 0.1). **Test-waits + ONE real perf fix; zero assertion weakening, zero CLS threshold-raise.** Surfaces: `e2e/f3.spec.ts` + `src/components/marketing/HeroSection.tsx` (the specific shifting element's component — NOT landing/page.tsx, NOT the ax1 spec).

### Per-flake root + fix
| Flake | Root (confirmed) | Fix (waits / reserve-space only) |
|---|---|---|
| **f3.spec.ts** signature-pad `data-has-ink` | The single mouse stroke occasionally didn't register ink before the assertion (canvas-draw timing). PORTAL-FND fixed the *positioning* (modal portaled to `<body>` → canvas on-screen) so it became *recoverable*, not deterministic; a flake AFTER the test bumps the waiver template v1→v2 then dooms the retry (the v1 check can't pass). | `signFlow`: wait for the pad to be **visible + scrolled into view**, then draw a **centred multi-segment stroke with stepped intermediate pointermoves** and **RE-DRAW until `data-has-ink="true"`**. The draw is idempotent (more strokes keep ink) and mutates no server state → re-running only the **interaction**, never the assertion (still `expect(...).toBe('true')`). A deterministic first sign means the v1→v2 bump can no longer doom a retry. |
| **ax1** landing CLS | The **Arabic web-font swap** — proven by isolation: blocking the woff2 on a local prod build gives **0 CLS** across 24 runs. The hero is `min-h-screen flex items-center`, so a load-time height change re-centres the whole block. Per-element, the **subheadline `<p>` is the SOLE element that changes height**: at `max-w-2xl` the size-adjusted fallback (narrower glyphs) fits the disciplines line in **1 row**, while IBM Plex Sans Arabic wraps to **2** (28px = the exact 677−649 content delta). `adjustFontFallback` matches vertical metrics but not glyph **advance widths**, so it can't prevent the re-wrap. | **Reserve 2 lines on that `<p>`** (`md:min-h-[3.5rem]`) so the fallback occupies the final height from first paint → the swap can't reflow it. Local prod build: **25/25 throttled runs < 0.0133, 0 CLS unthrottled** (was an intermittent 0.22). The CLS threshold (`< 0.1`) is **untouched** — this is a real perf fix. |

### Consecutive-green evidence (the stability bar — one green isn't proof)
The full suite went green **3× consecutively**, with `ax1` (`ax1-ar.spec.ts:42`) and `f3` (member `:81` + guardian `:132`) green in **every** run:
- **`27872663041`** — SUCCESS, 99 passed / 0 failed (31.0m) — https://github.com/TechStack2/proline-gym-platform/actions/runs/27872663041
- **`27874023170`** — SUCCESS, 99 passed / 0 failed (36.8m) — https://github.com/TechStack2/proline-gym-platform/actions/runs/27874023170
- **`27876077424`** — SUCCESS, 99 passed / 0 failed (36.1m) — https://github.com/TechStack2/proline-gym-platform/actions/runs/27876077424

### **f3 + ax1 stable across 3 consecutive greens; no assertion weakened, no CLS threshold raised: PASS**
...
## Cycle 6 / COACH-SHOWCASE-SEED — publish landing coaches in the demo reseed (2026-06-20)

**Mandate (demo gap):** COACH-LP's `get_landing_coaches()` reads `landing_visible` coaches, but `reseed_proline_demo()` (`000058`) never set any → the public showcase rendered **empty** in demos. Fold the publish into the reseed (demo-data only). FENCE: my surface = the new migration + reseed evidence ONLY; no touching `get_landing_coaches`/publish fns (COACH-LP) or any offline/Dexie/front-desk-read files (OFF-2).

### Build — migration `000060_coach_showcase_seed`
**REDEFINES `reseed_proline_demo()`** (self-contained `CREATE OR REPLACE` — `000058` was never merged to main, so this carries the full function body + the showcase block; `backup_proline_demo()` left untouched). After the gym seeds, a scoped block: `UPDATE coaches SET landing_visible=false WHERE gym_id=v_gym` (pristine reset → idempotent + no leak), then publishes a deterministic set keyed by the same `v_c1..v_c4` the reseed already resolves:
- **3 ACTIVE** (`landing_visible=true, landing_status='active'`) with `specialization_*` + `bio_*` populated trilingually (ar/en/fr), specialties matching each coach's seeded classes — c1 *Head Coach · MMA & Muay Thai*, c2 *Boxing & Kickboxing*, c3 *Muay Thai*.
- **1 COMING-SOON** (c4: `landing_visible=true, landing_status='coming_soon'`, *Strength & Conditioning*) — `landing_visible` true to match how `get_landing_coaches` projects coming-soon (it orders them last, not excludes them).
- The other 4 of 8 gym coaches stay unpublished. Non-destructive apply (only DEFINES the fn); wipe+reseed runs solely on the deliberate `SELECT`. Adds `landing_coaches` / `landing_coaches_active` / `landing_coaches_coming_soon` readiness keys to the return (the first via a read-only call to COACH-LP's reader).

### Apply + reseed evidence
- **VF apply `27887438599` — SUCCESS** (`-f apply=true -f migrations='000060_coach_showcase_seed'`): `apply 000060` → **HTTP 201**, `record 000060` → HTTP 201. Defines the updated fn only.
- **Deliberate reseed** `POST /rest/v1/rpc/reseed_proline_demo` (service key) → **HTTP 200**, returned: `landing_coaches=4, landing_coaches_active=3, landing_coaches_coming_soon=1, coaches_preserved=8` (4 of 8 published → rest unpublished), `members_total=20`, `other_tenant_students` unchanged by the scoped wipe.
- **Idempotency:** a 2nd reseed yields the identical published set (`4 / 3 / 1`).
- **Anon read path** `POST /rest/v1/rpc/get_landing_coaches` (anon key) → 4 coaches, each with bio: **Ali Fakih** (active · Boxing & Kickboxing), **Jawad Haitham** (active · Muay Thai), **Sami Haddad** (active · Head Coach · MMA & Muay Thai), **Rami Fakih** (coming_soon · Strength & Conditioning) — active-first alphabetical, coming-soon last. The other 4 coaches absent (no leak).
