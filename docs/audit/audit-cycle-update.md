...
### ⟶ new-password + confirm hold focus across keystrokes; validation/steps intact; /ar+/en: **PASS**
`tsc` clean. Targeted run [`28258671304`](https://github.com/TechStack2/proline-gym-platform/actions/runs/28258671304) (`-f projects="on1"`, setup+smoke+on1) — **success** (e2e job green; both ON-1 tests passed the new char-by-char `toHaveValue(full)` guard). The fix is **structural and locale-invariant** — no RTL/i18n path changed; the password step renders identically under `/ar` (FormWizard owns RTL; `Field` uses logical classes), so /ar holds by construction while the e2e guard runs on /en. **DO NOT MERGE — auditor merges.**
...
## Cycle 6 / INVITE-PHONE-UX — phone-as-username login

> **Branch:** `prompt-invite-phone-ux` off `main` (`fd0403b`) · Owner-chosen **Option B — NO Supabase auth-provider change** (phone provider stays disabled). The invite credentials a hidden synthetic email `m-<profileId>@members.proline.lb`; make the member's username their **phone**, the synthetic email stays the internal auth identifier.

### Design — phone is the username; synthetic email stays hidden
- **Invite UI shows the phone.** `invite.ts` already returns `waPhone` (= the profile's phone); [`invite-button.tsx`](../../src/components/shared/invite-button.tsx) now shows `waPhone` as the login + the wa.me message says "log in with {phone}". The synthetic email (`result.login`) is no longer surfaced to staff.
- **Server-side resolver (no enumeration oracle).** New server action [`src/lib/auth/actions.ts`](../../src/lib/auth/actions.ts) `signInWithPhone(phone, password)`: resolves phone → profile id via the **service-role** admin client (server-only via `import 'server-only'`, bypasses RLS, **never returned to the client**), builds the synthetic email `m-<id>@members.proline.lb`, and signs in via the **SSR client** (`@supabase/ssr`) — which writes the session cookies. The login page ([`login/page.tsx`](../../src/app/[locale]/auth/login/page.tsx)) routes a phone-shaped input to it; generic `loginError` on failure; on success `router.push('/dashboard') + refresh()` so the browser adopts the cookie session.
- **No enumeration / rate-limited:** the action ALWAYS performs a GoTrue sign-in — the resolved email when the phone matches, else a syntactically-valid non-existent one — so an unknown phone and a wrong password are indistinguishable (same generic result + timing) and both hit GoTrue's sign-in rate limit. There is **no anon `phone→email` RPC** (that would be the enumeration oracle the prompt forbids).
- **Backward compatible:** email sign-in unchanged (staff email + the synthetic email still work as a fallback). No Supabase provider change; **no migration** (the server-action + service-role path needs none — nothing to VF).

### ⟶ member logs in by phone+password; no enumeration; email login intact; no Supabase change; /ar+/en: **PASS**
**CI (targeted):** [run `28550388038`](https://github.com/TechStack2/proline-gym-platform/actions/runs/28550388038) — **SUCCESS** (20 passed, 40.2s, 0 flaky): **`on1`** member (16.3s) + coach (8.7s) — invite surfaces the **phone**, sign in **by phone** → onboarding → portal/coach; **`demo-guardian`** — **email** login → kid-switcher (email path intact); `auth-shell` renders. Migrations replayed clean (none added). tsc + `next build` clean.

### DRAG READ
- **The resolver is the whole trick.** Phone provider disabled → `signInWithPassword({phone})` can't work. Mapping phone → the hidden synthetic email inside a **service-role server action** (never an anon RPC) gives phone-as-username *without* an enumeration oracle and *without* a provider change. G1 later swaps the credential to phone-OTP — the adopted profile id (== the synthetic-email user's id) is unchanged, so it's a pure credential swap.
- **Cookie hydration:** the SSR server client sets the session cookies *inside* the action; `router.refresh()` makes the browser client adopt them — proven by on1 landing in the portal after a phone login (no extra round-trip).
- **Enumeration defence is behavioural, not a gate:** always-sign-in (real or dummy email) unifies timing + rate-limit + error, so there's nothing to special-case. If a future guarded resolver RPC is ever wanted it'd be additive — but this path needs none.
