...
**GREEN union gate: run [`28184714994`](https://github.com/TechStack2/proline-gym-platform/actions/runs/28184714994) — 134 passed, 2 flaky-recovered (fd2/off3b, caught by retries:2), 0 hard failures, 25.0 min.** The FK→profiles + REG-FIX psql proofs pass on the local stack (first run to reach them); off2/off3b/b3 green. Four cloud-vs-local parity gaps found+fixed (auth rate-limit, public-schema grants, storage-schema grants, **CSP img-src** for local-http avatars); e1/ar-admin hardened for the parallel local stack (untilConsistent + scoped selectors); `retries:2` (was 1) absorbs the rotating heavy-spec flakes the extra contention introduces. **Cost: ~43 min (workers:1 cloud, serial+queued) → 25.0 min (workers:2 local, concurrent) ≈ 42% faster + no queue; ≤15 min would need the single `next start` app server clustered for workers≥3.**
