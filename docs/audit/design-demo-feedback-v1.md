...
### 8. WhatsApp timing (operator answer, 2026-06-13) → G1 architecture locked
Client: excited about WhatsApp but worried about Cloud-API approval lead time — wants FULL functionality before the API exists, activation when approved. G1 therefore ships as a **channel abstraction with a per-gym toggle**: (a) **G1-bridge (no approval needed, works day 1):** wa.me deep-link share actions on staff surfaces — chase list, renewal reminders, invoice/receipt share, ON-1 credential share — open the staff member's OWN WhatsApp with a prefilled localized message; (b) **G1-full:** the Cloud API automates the same sends when the gym flips its WhatsApp settings to ACTIVE (credentials stored per-gym; status not-configured/pending/active). Nothing waits on Meta. **OPERATOR ACTION NOW: start the Meta Business verification + WhatsApp Cloud API application immediately** — approval takes weeks and runs fully in parallel.
