...
- 🔵 **IA-3 Schedule unification (ACTIVE):** prompt ISSUED — [`prompt-IA3-schedule-unification.md`](./prompt-IA3-schedule-unification.md). Week Timetable + Day Coach-diary (both calendar species per coach) + read-side PT conflict warning (non-blocking; zero write-path changes; zero schema). Auditor's sequencing call: IA-3 before B3 (already designed in the audit addendum; demo-critical; B3 needs design).
