...
- 🔵 **IA-2 Member-360 + Money (ACTIVE):** prompt ISSUED — [`prompt-IA2-member360-money.md`](./prompt-IA2-member360-money.md). Member file rebuilt with live panels (kills hardcoded `[]` props), Active|Prospects tabs (/leads re-homed), unified /money ledger, portal self-view card, i18n toast cleanup. Zero schema. Then auditor decides **B3 (family) vs IA-3 (Schedule unification: week timetable + coach day-diary + conflict guard — designed in the audit addendum)**.
