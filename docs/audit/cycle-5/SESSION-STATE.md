...
- 🔜 **PT-360 (operator-approved 2026-06-12, incl. Calendly-class booking + package-centric amendment):** design = [`../journey-pt-360.md`](../journey-pt-360.md). **PT-1** (catalog `pt_package_types` + desk sale RPC + landing PT section + **package-first presentation everywhere w/ billing tie** + refill thresholds→Inbox/nudge + hard-expire/extend + demo-residue cleanup) then **PT-2** (coach_availability + slot engine in gym tz + member instant-book + propose-a-time fallback + staff override). Then **D2 → D3 → F3 → G1 → E1 → G2** → V1 readiness review (incl. unscoped-reads re-audit) → deploy.
