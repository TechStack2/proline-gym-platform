...
- 🔵 **PARALLEL TRACK opened (2026-06-12):** REP-1 attendance history/reports repair + coach date picker runs CONCURRENTLY on an Opus session in worktree ../proline-rep1, port 3100, branch prompt-rep1-reporting — [`prompt-REP1-attendance-reporting.md`](./prompt-REP1-attendance-reporting.md). Rules: zero schema, disjoint surfaces only, namespaced i18n, auditor controls merge order (mainline lands first). Pulls the V1.1 reporting item into V1.
- 🔵 **FD-1 Front-Desk Cockpit (ACTIVE, mainline):** prompt ISSUED — [`prompt-FD1-front-desk-cockpit.md`](./prompt-FD1-front-desk-cockpit.md) (study: [`../design-front-desk-cockpit.md`](../design-front-desk-cockpit.md)). Today 2.0 ActionCard framework (expiring-memberships + money-today cards; collapse-when-zero; PT-1/ML-1 docking slots) + Member-360 contextual-action repair (wrong-target pills) + lists upgrade (phone search, badges, owing/expiring chips, prospect stages). Zero schema (seed-only tweak).
- 🔜 **PT-360 (operator-approved, incl. Calendly-class booking + package-centric amendment):** design = [`../journey-pt-360.md`](../journey-pt-360.md). **PT-1** (catalog + desk sale + package-first presentation w/ billing tie + refill→Today card + expiry) then **PT-2** (availability + slot engine + instant-book). Prompts pre-staged while prior slice runs (zero auditor latency). **PT-1 prompt PRE-STAGED:** [`prompt-PT1-catalog-sale-refill.md`](./prompt-PT1-catalog-sale-refill.md) — hand off ONLY after FD-1 merges (refill card docks into FD-1 ActionCard).
...
- **PARALLEL LANE FROZEN (2026-06-12, after 4 green slices: REP-1, ON-1-S, LPX-1, I18N-1 — all merged):** mainline-only from here (ON-1/G1/G2 territory). Opus worktree ../proline-rep1 may be removed.
**FRX backlog (I18N-1 key finding):** landing/portal components hardcode isRTL?ar:en, BYPASSING i18n — French falls back to English regardless of file completeness (HeroSection, PricingSection, TrialCTASection, FacilitySection, portal pages — full list in audit log). Component refactor; mainline mini-slice after PT-2 or readiness-review batch.
**ML-1 forks LOCKED (operator, 2026-06-12):** (1) plan change = next cycle, NO proration (manual discount bridges exceptions); (2) renewals AUTO-ISSUE N days pre-expiry (default 7) for BOTH monthly products + member nudge; (3) dunning = reminders due/+3d, then auto-LAPSE after configurable grace (default 7d past expiry) — check-in warning not door-block, owing/chase lists, one-tap reinstate; (4) freeze = bounded (max 30d/yr, min 7d chunk), extends end_date, auto-unfreeze, frozen excluded from renewal/dunning. Mechanism note: read-time state everywhere + ONE daily tick (pg_cron or equivalent) only for sends/flips — design doc next.
**OPERATOR 5-POINT DISPOSITION (2026-06-12, design: [`../design-uniform-experience.md`](../design-uniform-experience.md)):** [1] staff-created users→logins = ON-1 (scope now explicitly incl. coaches/staff invites); [2] coach trials island → loop-closure (coach marks outcome → pipeline stage + staff notify) = UX-2; [3] FormWizard convention on ALL entry forms = UX-2; [4] per-shell identity accents (member/coach/staff) = FRX; [5] settings PWA-nav bug + plans CRUD + belt-ladder editor = UX-2. Queue: PT-2 → ML-1 → **UX-2** → **FRX** → ON-1 → G1 → F3-lean → G2-lean → review → deploy.
**SQUEEZED QUEUE (updated 2026-06-12 — operator confirmed camps ARE selling now → E1 BUMPED):** FD-1 → PT-1 → **E1 camps (design: [`../journey-camps.md`](../journey-camps.md) — 90% pattern reuse; verify real camp-table columns first; register_camp atomic RPC w/ price snapshot + capacity FOR UPDATE; deposits = D1 partial payments natively; NO waitlist V1)** → PT-2 → **ML-1 (D2+D3 combined)** → ON-1 → G1 WhatsApp → **F3-lean (waiver/consent, no third-party e-sign)** → **G2-lean (offline ATTENDANCE only)** → V1 readiness review (incl. unscoped-reads re-audit) → deploy. Each slice docks its Today card + Member-360 action. PARALLEL lane after REP-1: ON-1 investigation spike → landing polish/SEO → fr i18n sweep; parallel FREEZES from ON-1 implementation onward.
