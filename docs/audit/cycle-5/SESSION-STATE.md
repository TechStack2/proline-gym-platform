...
- 🔜 **B3 family/household (queued next):** prompt issued — [`prompt-B3-family-household.md`](./prompt-B3-family-household.md) (forks locked inside; its migration takes the number after ADM-1's). Riders: <Toaster> mount, map query refinement if pin off. Then **D2 freeze/upgrade**.
- 🔜 **PT-360 journey deep-dive (auditor design in progress — operator mandate 2026-06-11):** PT is under-designed; master it end-to-end: marketing/landing → sale (package + coach + discipline) → allocation → scheduling (future date/time — the UX-1-flagged gap, absorbed here) → delivery (C1 ✅) → refill/renewal + expiry. Design doc + operator forks next; implementation slices follow B3.
