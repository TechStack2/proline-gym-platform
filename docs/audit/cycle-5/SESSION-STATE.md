...
- 🔵 **UX-1 bell fix + Add-Class wizard (ACTIVE, demo-critical):** prompt ISSUED — [`prompt-UX1-bell-class-wizard.md`](./prompt-UX1-bell-class-wizard.md). Operator hit both in manual testing: (a) bell realtime crash — channel topic `notifications:<uid>` reused across double-shell/strict-mode mounts → `.on()` after subscribe throws; fix = unique per-mount topic + race-safe cleanup. (b) AddClassModal: Day+Status still Radix Select (the B2-diagnosed non-opening class), dead room field, status default `'scheduled'` mismatches options, hardcoded strings → rebuild as touch-first wizard (chips/pills/steppers, no dropdowns), same insert path, zero schema. Next.js upgrade nag = post-V1 maintenance, out of scope.
