...
- 🔵 **IA — journey-centric admin redesign (ACTIVE; operator APPROVED the 7-workspace IA 2026-06-11):** IA-first + tenant-clean rule (white-label SaaS direction — Proline = tenant #1). Audit + IA + Schedule addendum (classes-timetable vs PT coach-diary, conflict guard; availability engine deferred V2): [`cohesion-audit-admin-ia.md`](./cohesion-audit-admin-ia.md). **IA-1 prompt ISSUED:** [`prompt-IA1-nav-inbox-today.md`](./prompt-IA1-nav-inbox-today.md) (nav 17→7 single-config + Inbox + Today + bell on 3 shells; recomposition only). Then **IA-2** (Member-360 + Money merge) → **IA-3** (Schedule unification: week timetable + coach day-diary) — IA-3 may swap after B3 at auditor's call.
