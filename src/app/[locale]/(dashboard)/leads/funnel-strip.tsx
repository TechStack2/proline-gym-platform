...
export async function FunnelStrip({ funnel, locale }: { funnel: Funnel; locale: string }) {
  ...
  const Table = ({ title, rows, testid }: { title: string; rows: Funnel['bySource']; testid: string }) => (
  ...
  return (
    <div className={cn('space-y-3', isRTL && 'rtl text-right')} data-testid="funnel-strip">
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <Table title={t('bySource')} rows={funnel.bySource} testid="funnel-by-source" />
        <Table title={t('byCampaign')} rows={funnel.byCampaign} testid="funnel-by-campaign" />
      </div>
    </div>
  )
}
