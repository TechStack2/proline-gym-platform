-- ============================================================
-- 000066: DEMO GUARDIAN — a 5th demo login (parent / kid-switcher)
-- PRO LINE Gym Platform / "Gym 360 Pro" (Cycle 6 / DEMO-GUARDIAN)
--
-- The demo login page offered only 4 quick-logins (owner/coach/reception/
-- student); the guardian feature (guardians + guardian_students, kid-switcher,
-- household billing, is_guardian_of RLS) is fully built but had no demo account.
-- This ADDITIVE, FORWARD-ONLY migration:
--   1. Creates the `guardian@prolinegym.lb` auth user IDEMPOTENTLY (same bcrypt
--      pattern as the 4 demo logins in 000008 — hashes app.demo_password).
--   2. Gives it user_roles('parent'), a profiles row, a guardians row, and a
--      guardian_students link to the HERO student (Karim, profile = the
--      student@prolinegym.lb login — created by 000017 from-zero, reused as
--      member #1 by reseed_proline_demo()).
--   3. Re-establishes that link inside reseed_proline_demo() so the demo guardian
--      survives a reseed (the WIPE drops guardian_students; the guardian profile
--      + guardians row are preserved, but the LINK must be recreated each reseed).
--
-- No RLS change — the guardian policies (is_guardian_of, students_guardian, …)
-- already exist (000037/000038); this only seeds data.
-- ============================================================

CREATE EXTENSION IF NOT EXISTS pgcrypto WITH SCHEMA extensions;

-- Default the demo password when unset, so this replays clean from ZERO
-- (`supabase db reset` / CI) exactly like 000008. An explicit ALTER DATABASE /
-- set_config (the long-lived cloud project) still wins.
DO $$
BEGIN
...
-- ---- Seed the guardian demo login (idempotent / guarded) ----
DO $$
DECLARE
  v_gym           uuid;
  v_guardian      uuid;
...
BEGIN
  ...
  IF v_gym IS NULL THEN
    RAISE NOTICE 'proline-gym not found — skipping demo guardian seed';
    RETURN;
  END IF;

  -- 1. Auth user (idempotent — same shape as the 000008 demo logins). The
  --    handle_new_user() trigger (000017) auto-creates a profiles row; gym_id is
  --    read from raw_user_meta_data, else falls back to the first active gym.
  SELECT id INTO v_guardian FROM auth.users WHERE email = 'guardian@prolinegym.lb';
  IF v_guardian IS NULL THEN
    ...
    VALUES (
      ...
      'authenticated', 'authenticated', 'guardian@prolinegym.lb',
      extensions.crypt(current_setting('app.demo_password'), extensions.gen_salt('bf')),
      now(), '{"provider":"email","providers":["email"]}',
      jsonb_build_object('gym_id', v_gym::text),
      now(), now(), '', '', '', ''
    )
    RETURNING id INTO v_guardian;
  END IF;

  -- 2. Profile (upsert — the trigger may have created it; set the demo identity).
  --    Same surname as the hero (Karim Mourad) → a believable parent.
  INSERT INTO profiles (id, gym_id, first_name_ar, first_name_en, first_name_fr,
    last_name_ar, last_name_en, last_name_fr, phone, gender, created_at, updated_at)
  VALUES (v_guardian, v_gym, 'سامر', 'Samer', 'Samer', 'مراد', 'Mourad', 'Mourad',
    '+96170100050', 'male', now(), now())
  ON CONFLICT (id) DO UPDATE SET
    gym_id        = EXCLUDED.gym_id,
    first_name_ar = EXCLUDED.first_name_ar, first_name_en = EXCLUDED.first_name_en, first_name_fr = EXCLUDED.first_name_fr,
    last_name_ar  = EXCLUDED.last_name_ar,  last_name_en  = EXCLUDED.last_name_en,  last_name_fr  = EXCLUDED.last_name_fr,
    phone         = EXCLUDED.phone, gender = EXCLUDED.gender, updated_at = now();

  -- 3. Parent role (idempotent).
  INSERT INTO user_roles (user_id, gym_id, role, is_primary)
  SELECT v_guardian, v_gym, 'parent', true
  WHERE NOT EXISTS (SELECT 1 FROM user_roles WHERE user_id = v_guardian AND role = 'parent');

  -- 4. Guardians row (idempotent — no UNIQUE on profile_id, so guard manually).
  SELECT id INTO v_guard_row FROM guardians WHERE profile_id = v_guardian AND gym_id = v_gym LIMIT 1;
  IF v_guard_row IS NULL THEN
    ...
    VALUES (v_guardian, v_gym, 'الأب', 'Father', 'Père', true)
    RETURNING id INTO v_guard_row;
  END IF;

  -- 5. Link to the hero student Karim (profile = the student@ login).
  SELECT id INTO v_student_login FROM auth.users WHERE email = 'student@prolinegym.lb';
  IF v_student_login IS NOT NULL THEN
    SELECT id INTO v_karim_stu FROM students WHERE profile_id = v_student_login AND gym_id = v_gym LIMIT 1;
    IF v_karim_stu IS NOT NULL THEN
      INSERT INTO guardian_students (guardian_id, student_id)
      VALUES (v_guard_row, v_karim_stu)
...
-- ============================================================
-- RESEED PARITY — re-establish the demo guardian's link on every reseed.
--
-- Full current body of reseed_proline_demo() (from 000060, the latest definer)
-- + a DEMO GUARDIAN re-link block before the coach showcase. CREATE OR REPLACE
-- needs the whole body; this is based on the CURRENT live definition so no later
