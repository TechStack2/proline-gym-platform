-- ============================================================
-- 000060: COACH SHOWCASE SEED — publish landing coaches in the demo reseed
-- PRO LINE Gym Platform / "Gym 360 Pro" (Cycle 6 / COACH-SHOWCASE-SEED)
--
-- COACH-LP's public landing showcase reads `landing_visible` coaches via
-- get_landing_coaches() (000059), but reseed_proline_demo() (000058) never set
-- any → the showcase renders EMPTY in demos. This migration REDEFINES
-- reseed_proline_demo() so that, after it seeds the gym, it also publishes a
-- deterministic showcase: 3 ACTIVE coaches (bio_* + specialization_* populated)
-- + 1 COMING-SOON teaser. The rest stay unpublished.
--
-- NON-DESTRUCTIVE APPLY: this only (re)DEFINES the function — applying the
-- migration never wipes. The wipe+reseed runs ONLY on a deliberate
-- `SELECT reseed_proline_demo();` (see [[demo-reseed-function]]).
--
-- SELF-CONTAINED: 000058 was never merged to main, so this carries the FULL
-- function body (a plain CREATE OR REPLACE) plus the showcase block — it does
-- not depend on 000058 being applied. backup_proline_demo() is unchanged and
-- lives in the prod DB untouched.
--
-- FENCE: this lane owns ONLY the showcase publish. It does NOT touch
-- get_landing_coaches / publish_coach_profile / set_coach_landing (COACH-LP owns
-- those) — it only CALLS get_landing_coaches read-only for the readiness count.
-- Scoped to proline-gym (the function already resolves v_gym).
-- ============================================================
