...
test('G1 · settings token-security + record-mode dispatch routing (active→sent, none→no-op, forced→failed no-rollback)', async ({ browser }) => {
  ...
  try {
    ...
    await page.goto('/en/settings?tab=messaging') // M2-A: WhatsApp lives in the Messaging section
    ...
    await page.goto('/en/settings?tab=messaging') // M2-A: re-check the token isn't exposed in the Messaging section
